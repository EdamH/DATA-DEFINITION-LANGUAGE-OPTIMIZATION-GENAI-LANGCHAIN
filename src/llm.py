...
def create_llm_pipeline_starcoder(OPTIMIZER_MODEL_ID):
    tokenizer = AutoTokenizer.from_pretrained(OPTIMIZER_MODEL_ID)
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token_id = tokenizer.eos_token_id
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = AutoModelForCausalLM.from_pretrained(
        OPTIMIZER_MODEL_ID, torch_dtype=torch.float16, device_map="auto" if device == "cuda" else None
    )
